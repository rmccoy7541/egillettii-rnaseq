...
def runModel(outFile, nuW_start, nuC_start, m12_start, T_start):
	...
	#fs = fs.sample()
        ...
        func = gillettii_models.split_unidirectional_migration
        params = array([nuW_start, nuC_start, m12_start, T_start])
        upper_bound = [10, 10, 100, 10]
        lower_bound = [1e-5, 1e-10, 0, 0]
        ...
        popt = dadi.Inference.optimize_log(p0, fs, func_ex, pts_l, upper_bound=upper_bound, lower_bound=lower_bound, maxiter=None, verbose=len(params), fixed_params=[None, None, None, None]) 
        ...
        s = str(nuW_start) + '\t' + str(nuC_start) + '\t' + str(m12_start) + '\t' + str(T_start) + '\t'
...
def mkOptionParser():
    ...
    usage = """%prog  <outFN> <nuW_start> <nuC_start> <m12_start> <T_start>
...
def main():
    ...
    m12_start    = float(args[3])
    T_start    = float(args[4])
    ...
    runModel(outFile, nuW_start, nuC_start, m12_start, T_start)
